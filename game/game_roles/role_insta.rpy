...
init -2 python:
    ...
    def dm_response_requirement(the_person):
        if renpy.random.randint(0,100) < 60 and not the_person.event_triggers_dict.get("insta_special_request_asap", False): #Respond at a random time, not as soon as possible.
            ...
            return False
        elif the_person.location == mc.location and mc.location.public:
            return False #If she's in the same location as us and we are in public she doesn't take the picture.
        elif mc.location == the_person.home and the_person.location == the_person.home: #Doesn't do it if she's at home and you're in the room with her (mainly for Lily/Mom)
            return False
        return True
...
label check_insta():
    ...
    call screen main_choice_display(build_menu_items(build_insta_menu(), draw_hearts_for_people = False, draw_person_previews = False))
...
label dm_option_underwear_response(the_person):
    ...
    if the_person.effective_sluttiness() < 10:
        ...
        "... And another."
        $ mc.change_locked_clarity(10)
        if the_person.get_opinion_score("showing her tits") > 0 and not the_person.tits_visible:
...
label insta_interrupt_check(the_person): # Returns Something???  a callback label or None. The callback should be called after the event.
    if the_person.has_role(sister_role):
        if the_person.location == mc.location and mc.location == the_person.home:
        ...
        elif the_person.location == the_person.home and mc.is_home:
