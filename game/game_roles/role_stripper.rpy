init -1 python:
    def stripper_private_dance_requirement(the_person):
        if not the_person.location == strip_club:
            return False
...
label stripper_private_dance_label(the_person):
    if the_person.has_role(cousin_role):
        ...
        menu:
            ...
            "Tits first\n{color=#ff0000}{size=18}Costs: $100{/size}{/color}" if not the_person.tits_visible:
        ...
        menu:
            ...
            "Stuff the cash into her cleavage\n{color=#ff0000}{size=18}Costs: $100{/size}{/color}" if the_person.outfit.get_upper_top_layer:
                "You pull out your wallet and retrieve [the_person.title]'s money."
                $ mc.business.change_funds(-100)
...
label stripper_offer_hire(the_person):
    ...
    if get_strip_club_foreclosed_stage() == 0 and the_person.location == strip_club:
...
label stripper_hire_accept(the_person):
    ...
    if _return:
        ...
        if strip_club_is_closed() or get_strip_club_foreclosed_stage() >= 5:
