...
label model_photography_list_label(the_person):
    ...
    if not the_person.outfit.are_panties_visible and not the_person.outfit.is_bra_visible: #She has underwear on and something over both.
        $ outfit_state = 0 #She's wearing enough we can have a "strip to your underwear" scene.
    elif the_person.outfit.is_bra_visible or the_person.outfit.are_panties_visible or (not the_person.tits_visible and not the_person.vagina_visible):
...
label photo_be_sexy(the_person):
    ...
    if not the_person.outfit.are_panties_visible and not the_person.outfit.is_bra_visible: #She has underwear on and something over both.
        $ outfit_state = 0 #She's wearing enough we can have a "strip to your underwear" scene.
    elif not the_person.outfit.is_bra_visible or not the_person.outfit.are_panties_visible or (not the_person.tits_visible and not the_person.vagina_visible):
        $ outfit_state = 1 #She's wearing enough that we can have a strip scene.
    else:
        $ outfit_state = 2 #She's practically naked with no clothing on.
    menu:
        "Strip to your underwear" if slut_willingness+skimpy_uniform_bonus >= 30 and outfit_state == 0: #TODO: Also check to make sure she's got the right type of clothing TO strip down to her underwear
...
label photo_touch(the_person):
    ...
    menu:
        ...
        "Take photos as she climaxes":
            ...
            return the_person.outfit.outfit_slut_score + (the_person.sex_skills["Foreplay"] * 5)
    return _return
...
label photo_blowjob(the_person):
    ...
    menu:
        ...
        "Take photos as you cum":
            ...
            return the_person.outfit.outfit_slut_score + 10 + (5* the_person.sex_skills["Oral"])
    return _return
...
label photo_sex(the_person):
    ...
    if the_choice == "Pull out and cum":
        ...
        if mc.condom:
            ...
            $ climax_controller.do_clarity_release(the_person)
            "You stay tight against her while you pump your hot load deep inside of her pussy."
            $ mc.change_locked_clarity(10)
            $ the_person.draw_person(position = "missionary")
            $ play_moan_sound()
    ...
    $ mc.condom = False
    return the_person.outfit.outfit_slut_score + 15 + (5* the_person.sex_skills["Vaginal"]) + came_inside_mod
