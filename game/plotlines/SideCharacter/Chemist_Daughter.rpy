...
init -1 python: #Requirement functions
    ...
    def chemist_daughter_after_raise_consult_requirement():
        ...
        if person is None:
            return True
        if person.salary > person.event_triggers_dict.get("starting_pay", 0) and \
            person.days_since_event("last_raise") > TIER_1_TIME_DELAY:
            return True
        return False
    ...
    def chemist_daughter_daddy_title_requirement(person):
        return person.sluttiness > 40 \
            and mc.is_at_work and mc.business.is_open_for_business
