...
init 700:
    python:
        ...
        def introvert_possessive_titles(person):
            return introvert_titles(person)
...
label introvert_flirt_response_high(the_person):
    if mc.location.person_count > 1 and the_person.effective_sluttiness("kissing") < (25 - (5*the_person.get_opinion_score("public_sex"))): # There are other people here, if she's not slutty she asks if you want to find somewhere quiet
        ...
        menu:
            "Kiss her":
                ...
                call fuck_person(the_person, start_position = kissing, private = mc.location.person_count < 2, skip_intro = True) from _call_fuck_person_58
                $ the_person.call_dialogue("sex_review", the_report = _return)
                $ the_person.review_outfit()
            "Just flirt":
                $ the_person.draw_person()
    ...
    if the_person.wants_creampie:
        if the_person.knows_pregnant:
            if the_person.relationship != "Single":
                ...
                the_person "I'm so happy someone is still fucking me! My [so_title] lost all interest in me after I got pregnant."
            else:
