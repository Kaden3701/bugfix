...
init 700:
    python:
        ...
        def reserved_possessive_titles(person):
            return reserved_titles(person)
...
label reserved_flirt_response_mid(the_person):
    if the_person.is_wearing_uniform and the_person.has_role(employee_role):
        ...
        if the_person.effective_sluttiness() < 20 and mc.location.person_count > 1:
            ...
            if not the_person.outfit.wearing_panties and not the_person.vagina_visible: #Not wearing underwear, but you can't see so she coments on it.
...
label reserved_flirt_response_high(the_person):
    if mc.location.person_count > 1 and the_person.effective_sluttiness() < (25 - (5*the_person.get_opinion_score("public_sex"))): # There are other people here, if she's not slutty she asks if you want to find somewhere quiet
        ...
        menu:
            "Kiss her":
                ...
                call fuck_person(the_person, start_position = kissing, private = mc.location.person_count < 2, skip_intro = True) from _call_fuck_person_52
                $ the_person.call_dialogue("sex_review", the_report = _return)
                $ the_person.review_outfit()
