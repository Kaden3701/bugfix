...
init 700:
    python:
        def bimbo_titles(person):
            return person.name
        def bimbo_possessive_titles(person):
            return bimbo_titles(person)
        def bimbo_player_titles(person):
...
label bimbo_flirt_response_high(the_person):
    if mc.location.person_count > 1 and the_person.effective_sluttiness() < (25 - (5*the_person.get_opinion_score("public_sex"))):
        ...
        menu:
            "Kiss her":
                ...
                call fuck_person(the_person, start_position = kissing, private = mc.location.person_count < 2, skip_intro = True) from _call_fuck_person_56
                $ the_person.call_dialogue("sex_review", the_report = _return)
                $ the_person.review_outfit()
            "Just flirt":
