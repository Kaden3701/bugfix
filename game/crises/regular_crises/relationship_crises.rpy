...
init -1 python:
    ...
    def get_so_relationship_improve_person():
        potential_people = []
        for person in [x for x in mc.phone.get_person_list(excluded_people = unique_character_list + side_character_unavail_list())
...
label affair_dick_pick_label():
    ...
    $ the_person.set_event_day("affair_dick_pick")
    $ mc.end_text_convo()
    $ clear_scene()
    return
...
label girlfriend_nudes_label():
    ...
    $ mc.end_text_convo()
    $ clear_scene()
    $ the_person.set_event_day("girlfriend nudes")
    return
