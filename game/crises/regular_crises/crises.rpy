...
label get_drink_crisis_label():
    ...
    the_person "Sure. [the_person.coffee_style], please."
    $ clear_scene()
    "You nod and head to the little break room in the office. It doesn't take you long to have both of your drinks made up."
    menu:
        "Add a dose of serum to [the_person.title]'s drink" if mc.inventory.total_serum_count > 0:
            ...
            if the_person.effective_sluttiness() < 20:
                ...
                if len(the_person.outfit.get_lower_ordered()) > 0: #ie. she's wearing something to take off
                    ...
                    if the_person.judge_outfit(test_outfit):
                        ...
                        $ the_person.draw_person(position = "walking_away")
                        "[the_person.possessive_title] grabs her [the_item.display_name], turns back to the shelf, and finally finds the binder she was looking for. She takes it and walks past you, making sure to shake her ass as you watch."
...
label special_training_crisis_label():
    ...
    $ the_person = get_random_from_list(mc.business.employee_list)
...
label water_spill_crisis_label():
    ...
    $ the_person.draw_person(emotion="angry")
    $ the_person.call_dialogue("surprised_exclaim")
    "She tries to wipe the water off, but not before it's soaked through the front of her [the_clothing.display_name]."
    $ mc.change_locked_clarity(10)
...
init 1 python:
    ...
    def home_fuck_crisis_get_person():
        meets_sluttiness_list = [x for x in mc.business.employee_list if x.sluttiness > 15 and not x.is_family and not x.has_role(girlfriend_role) and x.relationship == "Single" and x.get_opinion_score("cheating on men") > 0]
...
label quitting_crisis_label(the_person): #The person tries to quit, you have a chance to keep her around for a hefty raise (Or by fucking her, if her sluttiness is high enough).
    ...
    if the_person.job_happiness_score >= 0:
    ...
    if mc.is_at_work: #If you're already in your office just kick back and relax.
        $ mc.change_location(ceo_office)
    ...
    menu:
        "Offer a raise":
            ...
            $ deficit = -the_person.job_happiness_score
...
init 1 python:
    def invest_opportunity_crisis_requirement():
        return mc.business.research_tier > 0 and mc.business.is_open_for_business and mc.is_at_work
...
label invest_rep_visit_label(rep_name):
    ...
    if time_of_day == 3:
        ...
        if mc.business.employee_count == 0:
            ...
            $ success_chance = 10
            $ flirt_requires_string = "Flirt with " + rep_name + "\n{color=#ff0000}{size=18}Requires: Obedience 110, " + get_gold_heart(20) + "{/size}{/color}"
            $ seduce_requires_string = "Seduce " + rep_name + "\n{color=#ff0000}{size=18}Requires: Obedience 130, " + get_gold_heart(60) + "{/size}{/color}" #TODO: check to make sure that the sluttiness requirement is being shown.
            ...
            menu:
                "Impress [rep_name]": #Simplest option, just positive talk about the company.
                    ...
                    $ success_chance += helper.outfit.outfit_slut_score/5 #Our success chance is based on the impressing persons charisma and marketing, with a small bonus based on their outfit's sluttiness.
                ...
                "Flirt with [rep_name]" if helper.sluttiness >= 20 and helper.obedience >= 110: #Requires some sluttiness, more effective than impress.
                    ...
                    $ success_chance += helper.outfit.outfit_slut_score/4 #Same basic calculations as above but both outfit sluttiness and charisma/skill are more effective.
                ...
                "Seduce [rep_name]" if helper.sluttiness >= 60 and helper.obedience >= 130: #Take rep_name off screen and "convince" him to invest in your company. Highest effectiveness but requires high levels of sluttiness and obedience.
                    ...
                    $ success_chance += helper.outfit.outfit_slut_score/3 # Higher outfit contribution and...
                    python:
            ...
            "Talk about her body":
                ...
                if mc.location.person_count <= 1:
                    ...
                    if the_person.get_opinion_score("showing her tits") > 0:
                        ...
                        if top_clothing:
                            "[the_person.possessive_title] pulls at her [top_clothing.display_name], sliding it down a little bit as if she's about to remove it."
        ...
        if mc.location.person_count <= 1:
            if not the_person.get_opinion_score("public sex") < 0:
                ...
                if the_person.vagina_available:
                    ...
                    if the_item:
                        $ play_moan_sound()
                        "[the_person.title] rolls her chair back from the desk and slides a hand inside of her [the_item.display_name]. She bites her lip and moans quietly to herself."
                    $ the_item = None
            ...
            if not the_person.get_opinion_score("public sex") < 0:
                ...
                if the_person.vagina_available:
                    ...
                    $ the_item = the_person.outfit.get_lower_top_layer
                    if the_item:
                        "[the_person.possessive_title] has a hand down her [the_item.display_name]. You can see one of her fingers making little movements under the fabric as she touches herself."
                    $ the_item = None
    ...
    menu:
        ...
        "Stay silent and let them fight it out":
            ...
            if person_one.sluttiness < 40 or person_two.sluttiness < 40:
                ...
                $ the_clothing = loser.choose_strip_clothing_item()
                if the_clothing:
                    "While they fight [winner.title] gets a hold of [loser.title]'s [the_clothing.display_name]. She tugs on it hard while she swings [loser.title] around and there's a loud rip."
                    ...
                    "[loser.title] grabs her [the_clothing.display_name] and hurries off to find somewhere private."
                ...
                while the_clothing and loser.outfit.outfit_slut_score < 80:
                    $ ran_num = renpy.random.randint(0,3)
                    if ran_num == 0:
                        "[winner.title] grabs [loser.title] by the [the_clothing.display_name] and yanks her around. There's a loud rip and the piece of clothing comes free."
                        $ scene_manager.draw_animated_removal(loser, the_clothing)
                        loser "You bitch!"
                    elif ran_num == 1:
                        "[loser.title] circles around [winner.title], then runs forward yelling and screaming. [winner.title] pushes her to the side, then grabs her by the [the_clothing.display_name] and tries to pull her to the ground."
                        "The girls struggle until [loser.title]'s [the_clothing.display_name] comes free and they separate. [winner.title] drops it to the ground."
                        $ scene_manager.draw_animated_removal(loser, the_clothing)
                        loser "You'll pay for that, slut!"
                    elif ran_num == 2:
                        "[winner.title] and [loser.title] collide, screaming profanities at each other."
                        "You aren't sure exactly what happens, but when they separate [winner.title] is holding a piece of fabric that used to be [loser.title]'s [the_clothing.display_name]."
                        $ scene_manager.draw_animated_removal(loser, the_clothing)
                        loser "Is that all you've got?"
                    else: #ran_num == 3
                        "[loser.title] gets an arm around [winner.title]'s waist and pushes her against a desk. The two grapple for a moment, then [winner.title] grabs [loser.title] by the [the_clothing.display_name] and pulls until the piece of clothing rips off."
                        $ scene_manager.draw_animated_removal(loser, the_clothing)
...
init 1 python:
    def research_reminder_crisis_requirement():
        if anyone_else_in_office() and not mc.business.head_researcher is None:
            if mc.business.active_research_design is None and mc.business.event_triggers_dict.get("no_research", 0) > 8:
...
init 1 python:
    ...
    def get_random_mother_from_company_with_children():
        valid_people_list = []
        for person in [x for x in mc.business.employee_list if x.age >= 34 and not side_character_is_unavail(x) and x not in unique_character_list]:
                ...
                menu:
                    ...
                    "Make her suck you off":
                        ...
                        if the_person.is_willing(blowjob, private = False):
                            if (the_person.get_opinion_score("public sex") > 0 and mc.location.person_count > 1) or the_person.get_opinion_score("giving blowjobs") > 0:
                    ...
                    "Make her fuck you":
                        ...
                        if the_person.is_willing(missionary, private = False):
                            ...
                            if mc.location.person_count > 1 and the_person.effective_sluttiness() < (80 - 10*the_person.get_opinion_score("public sex")):
