init -1 python:
    def sleeping_walk_in_requirement(the_person):
        if not (time_of_day == 0 or time_of_day == 4): #ie. early morning (sleeping in) or late at night (early bed time)
            return False
        if the_person == mom and time_of_day == 0 and day%7 == 5:
            return False #Don't want to trigger at the same time as the morning offer.
        if not the_person.location == the_person.home:
            return False
        if not the_person.has_role([sister_role, mother_role]):
            return False
        if the_person.home.person_count > 1: #Nobody else in the room.
            return False
        return True

    def reset_sleeping_walkin_event(person):
        found = next((x[0] for x in limited_time_event_pool if x[0].effect == "sleeping_walk_in_label"), None)
        if found:
            person.on_room_enter_event_list.append(
                Limited_Time_Action(found, 1)   # just one more turn
            )

    limited_time_event_pool.append([
        Action("Sleeping walk in", sleeping_walk_in_requirement, "sleeping_walk_in_label", event_duration = 1),
        8, "on_enter"])
                ...
                "Sleep in her bed {image=gui/heart/Time_Advance.png}{image=gui/heart/Time_Advance.png}" if time_of_day == 4: #Only at night #TODO: Break the "sleep with" events out for more detail
                    ...
                    if the_person == mom:
                        ...
                        if the_person.love < 15:
                            ...
                            $ reset_sleeping_walkin_event(the_person)
                        ...
                        if the_person.love < 25:
                            ...
                            the_person "Go bug mom, maybe she'll let you stay with her."
    ...
    menu:
        ...
        "Grope her tits -5{image=gui/extra_images/energy_token.png}" if mc.energy >= 5 and the_person.effective_sluttiness() >= grope_tits_slut_requirement:
            ...
            if the_person.arousal_perc > 100:
                ...
                $ the_person.have_orgasm(half_arousal = True, trance_chance_modifier = 2 * the_person.get_opinion_score("being fingered"))
        ...
        "Tit fuck her\n{color=#ff0000}{size=18}Requires: [titfuck_slut_token]{/size}{/color} (disabled)" if the_person.has_large_tits and the_person.tits_available and masturbating and the_person.effective_sluttiness() < titfuck_slut_requirement:
            pass
        ...
        "Fuck her\n{color=#ff0000}{size=18}Requires: [fuck_slut_token]{/size}{/color} (disabled)" if the_person.vagina_available and masturbating and the_person.effective_sluttiness() < fuck_slut_requirement:
            pass
...
label sleep_climax_manager(the_person, straddle = False, stomach_allowed = False, face_allowed = False, tits_allowed = False, throat_allowed = False, inside_allowed = False): #Helper that collects all of the orgasm checks
    ...
    $ cum_tits_slut_token = get_gold_heart(cum_tits_slut_requirement)

    $ cum_face_slut_requirement = 40
    $ cum_face_slut_token = get_gold_heart(cum_face_slut_requirement)

    $ cum_throat_slut_requirement = 55
    $ cum_throat_slut_token = get_gold_heart(cum_throat_slut_requirement)

    $ cum_inside_slut_requirement = 65
    $ cum_inside_slut_token = get_gold_heart(cum_inside_slut_requirement)
