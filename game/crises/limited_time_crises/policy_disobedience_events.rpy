...
init -1 python:
    def uniform_disobedience_on_move(uniform_disobedience_priority): #This is an on_move function called by the business on_move phase. It is only run once, by the uniform policy with the highest priority
        ...
        # print("Running uniform priority {}".format(highest_active_priority))

        for person in [x for x in mc.business.employee_list if x.should_wear_uniform and x.planned_uniform and x.is_wearing_uniform and x.days_since_event("uniform_disobedience", True) > 3]:
            ...
            if not person.judge_outfit(person.planned_uniform):
                ...
                disobedience_chance += -2*(person.get_opinion_score("skimpy uniforms"))
            else:
                ...
                disobedience_chance += -2*(person.get_opinion_score("work uniforms"))

            # print("{} {} disobedience chance {}%".format(person.name, person.last_name, disobedience_chance))

            if renpy.random.randint(0,100) < __builtin__.max(disobedience_chance, 2): # minimum chance is 2%
                ...
                person.set_uniform(person.planned_outfit, wear_now = True) #Overwrites the uniform they intended to wear for the day, so the next Move doesn't change it but the end of day will.

        return

    def uniform_disobedience_requirement(the_person):
        return mc.business.is_open_for_business and time_of_day in [1,3] \
            and the_person.should_wear_uniform and the_person.planned_uniform == the_person.planned_outfit
...
label uniform_disobedience_event(planned_uniform, the_person):
    ...
    $ the_person.set_event_day("uniform_disobedience")
    ...
    menu:
        "Send her to get changed":
            ...
            $ the_person.set_uniform(planned_uniform)
            $ the_person.draw_person()
