...
init -1 python:
    ...
    def work_spank_opportunity_requirement(the_person):
        ...
        return the_person.is_at_work and (the_person.is_employee or the_person.is_strip_club_employee or the_person.is_intern)

    def ask_new_title_requirement(the_person):
        if the_person.obedience > 180: #If she has higher obedience she ONLY lets you change her title.
            return False
        if the_person.days_since_event("ask_new_title", True) < TIER_2_TIME_DELAY:    # only once every two weeks
            return False
    ...
    def work_walk_in_requirement(the_person): #AKA she has to work for you, be at work, and be turned on
        ...
        if the_person.arousal_perc < 70:
            return False
        if the_person.effective_sluttiness() < 40 - (5*the_person.get_opinion_score("masturbating")):
            return False
        if not the_person.has_role(employee_role):
            return False
        if not the_person.is_at_work:
            return False
        if the_person not in mc.business.employee_list:
            return False
        return True

    def new_insta_account_requirement(the_person):
        if the_person.love < 15: #Girls who don't like you won't tell you they've made a profile (and are assumed to either have one or not depending on their starting generation)
            return False
        if the_person.effective_sluttiness() < (100 - the_person.personality.insta_chance) - 5 * the_person.get_opinion_score("showing her tits") - 5 * the_person.get_opinion_score("showing her ass"):
            return False #Personality type and Opinions has a large impact on chance to generate a new profile.
        if the_person.has_role([instapic_role, mother_role, sister_role]):
            return False
        return True

    def new_dikdok_account_requirement(the_person):
        if the_person.love < 30: #Girls who don't like you won't tell you they've made a profile (and are assumed to either have one or not depending on their starting generation)
            return False
        if the_person.event_triggers_dict.get("block_dikdok", False): # you asked her to quit DikDok
            return False
        if the_person.effective_sluttiness() < (100 - the_person.personality.dikdok_chance) - 5 * the_person.get_opinion_score("showing her tits") - 5 * the_person.get_opinion_score("showing her ass"):
            return False #Personality type and Opinions has a large impact on chance to generate a new profile.
        if the_person.has_role([dikdok_role, mother_role, sister_role, girlfriend_role]):
            return False
        return True

    def new_onlyfans_account_requirement(the_person):
        if the_person.love < 40: #Girls who don't like you won't tell you they've made a profile (and are assumed to either have one or not depending on their starting generation)
            return False
        if the_person.event_triggers_dict.get("block_onlyfans", False): # you asked her to quit Only Fans
            return False
        if the_person.effective_sluttiness() < 80 - 10 * the_person.get_opinion_score("showing her tits") - 5 * the_person.get_opinion_score("showing her ass") - 5 * the_person.get_opinion_score("public sex"):
            return False
        if the_person.has_role([mother_role, sister_role, girlfriend_role, onlyfans_role]):
            return False
        return True
...
label ask_new_title_label(the_person):
    ...
    if renpy.random.randint(0, 1) == 1: # 50/50
