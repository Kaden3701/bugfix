...
init -2 python:
    ...
    def attention_seize_inventory_requirement(the_person):
        if mc.business.inventory.total_serum_count >= 10:
            return True
        return False
...
label attention_event():
    ...
    if mc.business.event_triggers_dict.get("attention_times_visited", 0) < 2:
        $ city_rep.planned_outfit = city_rep.event_triggers_dict.get("city_rep_forced_uniform", city_rep.decide_on_outfit())
    else:
        $ city_rep.planned_outfit = city_rep.event_triggers_dict.get("city_rep_forced_uniform", city_rep.decide_on_outfit(.2))

    $ city_rep.apply_planned_outfit()
...
label attention_seize_inventory(the_person):
    ...
    $ mc.log_event("{} doses of {} seized!".format(doses_seized, highest_attention_design.name), "float_text_red")
...
label attention_illegal_serum(the_person):
    ...
    $ mc.log_event("{} made illegal: +{} Attention!".format(highest_attention_design.name, highest_attention_design_change), "float_text_red")
    $ highest_attention_design = None
    return
...
label attention_coffee(the_person):
    ...
    if the_person.love < 0:
        ...
        menu:
            "Add a dose of serum to her coffee" if mc.inventory.total_serum_count > 0:
            ...
            "Add a dose of serum to her coffee\n{color=#ff0000}{size=18}Requires: Serum in Inventory{/size}{/color} (disabled)" if mc.inventory.total_serum_count <= 0:
                pass
