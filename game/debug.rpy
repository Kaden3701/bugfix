...
init -15 python:
    ...
    def verify_wardrobe_report(the_wardrobe = None):
        ...
        for outfit in the_wardrobe.underwear_sets:
            for cloth in outfit:
                if cloth.layer >= 2:
                    write_log("Problem: {} is stored as underwear set but contains layer 2 items.".format(outfit.name))
                    break

        for outfit in the_wardrobe.overwear_sets:
            for cloth in outfit:
                if cloth.layer < 2:
                    write_log("Problem: {} is stored as overwear set but contains layer 1 items.".format(outfit.name))
                    break
