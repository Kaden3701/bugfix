...
init 2 python:
    ...
    def create_alexia_character():
        ...
        alexia_barista_job = Job("Barista", critical_job_role, job_location = coffee_shop)
        ...
        alexia.set_opinion("kissing", 1, False)  # she likes kissing
        alexia.set_opinion("flirting", 1, False)  # she likes kissing
        alexia.set_opinion("cheating on men", -2, False)  # she loves her boyfriend
...
init -2 python:
    ...
    def add_alexia_phase_two_action(the_person):
        ...
        the_person.get_role_reference(alexia_role).add_action(alexia_intro_phase_two_action)
        return
...
label alexia_phase_zero_label():
    $ alexia.set_override_schedule(downtown, the_times = [1, 2, 3]) #Let her wander Downtown so we can start quest chain.
    return
...
label alexia_intro_phase_two_label(the_person):
    ...
    $ clear_scene()
    "[the_person.possessive_title] goes into the back to change her clothes and together you walk out."
