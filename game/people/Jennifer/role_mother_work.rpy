...
init -2 python:
    def mom_work_promotion_one_requirement(the_person):
        if mc.business.is_weekend:
            return False
        if the_person.sluttiness < 20:  #Now based solely on sluttiness
            return False
        if the_person.location != mom_bedroom or the_person.has_queued_event("sleeping_walk_in_label"):
            return False
    ...
    def mom_work_secretary_replacement_bigger_tits_reintro_requirement(the_person):
        if mc.business.is_weekend or not the_person.has_job(mom_secretary_job):
            return False
        ...
        if the_person.event_triggers_dict.get("mom_office_slutty_level", 0) != 1:
            return False
        if the_person.event_triggers_dict.get("mom_replacement_approach", "seduce") != "tits":
            return False
        if (the_person.event_triggers_dict.get("mom_replacement_approach_waiting_for_tits", False) and the_person.rank_tits(the_person.tits) <= the_person.rank_tits(the_person.event_triggers_dict.get("mom_replacement_approach_waiting_for_tits", False))):
            return False
        if Person.rank_tits(the_person.tits) >= 8:  # they are already big enough
            return False
    ...
    def mom_work_promotion_one_before_requirement(the_person, start_day):
        if mc.business.is_weekend or day < start_day or not the_person.has_job(mom_associate_job):
            return False
...
label mom_work_promotion_one(the_person): #Mom is up for a promotion and asks you for help. You can tell her to get it on her merits (She doesn't) or to dress up for it.
    # Triggered when Mom has 20 sluttiness, 15 Love, and at least 100 Obedience
...
init 2 python:
    ...
    def mom_work_promotion_one_report_requirement(the_person, start_day):
        if not the_person.has_job(mom_associate_job):
            return False
        elif not the_person.is_at_mc_house: # only talk at home
            return False
...
label mom_work_promotion_two_intro(the_person): # She asks you to help her prepare for her one-on-one interview.
    ...
    $ the_person.progress.lust_step = 2
...
init 2 python:
    def mom_work_promotion_two_report_requirement(the_person):
        if not the_person.has_job(mom_associate_job):
            return False
        elif not the_person.is_at_mc_house: #Only talk about this at home
            return False
        return True
...
init 2 python:
    def mom_work_secretary_replacement_intro_requirement(the_person, the_day):
        if not the_person.has_job(mom_secretary_job):
            ...
            return False
        elif the_person.effective_sluttiness() < 40:    #This is now a 40 sluttiness event for Jennifer
            return False
        return True
...
label mom_work_promotion_two_report(the_person): #TODO: Hook this up as an on_room or maybe a mandatory event
    $ the_person.progress.lust_step = 3
...
label mom_work_secretary_replacement_bigger_tits_options(the_person):
    menu:
        ...
        "Give her some serum" if mc.inventory.total_serum_count > 0: #Note: The seduce event has already been added and is watching her breast size, so if it goes up she'll trigger it right away
        ...
        "Give her some serum\n{color=#ff0000}{size=18}Requires: Serum Dose{/size}{/color} (disabled)"if mc.inventory.total_serum_count <= 0:
            pass
...
label mom_work_secretary_replacement_report(the_person):
    ...
    $ the_person.change_happiness(20, add_to_log = False)
    $ the_person.progress.lust_step = 5
    ...
    $ the_person.add_unique_on_room_enter_event(mom_lust_story_bridge)
...
label mom_promotion_boss_phase_one(the_secretary):
    ...
    menu:
        ...
        "Reference [mom.title]":
            mc.name "Mr. [mc.last_name]. I'm the son of [mom.fname], there's a family matter I need to discuss with him."
...
label mom_convince_quit_label(the_person):
    ...
    "She will no longer be able to transformed into the office slut... but maybe you could take the place of her boss?"
    menu:
        ...
        "You can work for me" if mc.business.employee_count < mc.business.max_employee_count:
        ...
        "You can work for me\n{color=#ff0000}{size=18}Requires: Free employee slot{/size}{/color} (disabled)" if mc.business.employee_count >= mc.business.max_employee_count:
            pass
