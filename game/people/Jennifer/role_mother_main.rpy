...
init 2 python:
    ...
    def get_mother_role_actions():
        ...
        mom_girlfriend_underwear_shopping_action = Action("Shop for new lingerie {image=gui/heart/Time_Advance.png}", girlfriend_underwear_shopping_requirement , "girlfriend_underwear_shopping_label",
        ...
        return [mother_offer_make_dinner, mom_work_promotion_two_prep_action, mom_work_bigger_tits_reintro, mom_sister_girlfriend_blessing_action, mom_girlfriend_return_action, mom_girlfriend_underwear_shopping_action, fetish_mom_kitchen_action]
    ...
    def jennifer_oral_position_filter(oral_positions):
        return mom.event_triggers_dict.get("kissing_revisit_complete", False)

    def jennifer_vaginal_position_filter(vaginal_positions):
        return mom.event_triggers_dict.get("anal_revisit_complete", False)

    def jennifer_anal_position_filter(anal_positions):
...
init -2 python:
    ...
    def mom_offer_make_dinner_requirement(the_person):
        return time_of_day == 3 and the_person.location == kitchen

    def mom_date_intercept_requirement(the_person, the_date):
        if the_person is the_date:
            return False
        if not the_person.is_at_mc_house:
            return False
...
label mom_weekly_pay_label(the_person):
    ...
    if the_person.effective_sluttiness() < 40:
        ...
        if mc.business.event_triggers_dict.get("Mom_Payment_Level",0) >= 1: #We've been through this song and dance already.
            ...
            $ the_person.change_stats(happiness = 5, love = 2, slut = 2, max_slut = 30)
...
label mom_low_sluttiness_weekly_pay(the_person):
    call screen main_choice_display(build_menu_items([build_mom_low_sluttiness_weekly_menu()]))
...
init 5 python:
    def get_mom_high_sluttiness_weekly_actions():
        return [mom_weekly_pay_strip_for_me_action,
            ...
            mom_weekly_pay_tit_fuck_action,
    ...
    def mom_weekly_pay_tit_fuck_requirement(amount):
        if not mc.business.has_funds(amount):
            return "Requires: ${}".format(amount)
        if mom.effective_sluttiness("touching_body") < 40:
            return "Not slutty enough"
        if mom.get_opinion_score("giving tit fucks") <= -2:
            return "She hates tit fucks"
        return True
    ...
    mom_weekly_pay_tit_fuck_action = Action("Give me a tit fuck\n{color=#ff0000}{size=18}Costs: $200{/size}{/color}", mom_weekly_pay_tit_fuck_requirement, "mom_weekly_pay_tit_fuck_label", requirement_args = 200)
...
label mom_high_sluttiness_weekly_pay(the_person): #TODO: Change all of these over to use Actions instead of just being a menu.
    call screen main_choice_display(build_menu_items([build_mom_high_sluttiness_weekly_menu()]))
    if isinstance(_return, Action):
        $ _return.call_action()
    return
...
label mom_weekly_pay_give_blowjob():
    ...
    call mc_sex_request(the_person, the_request = "blowjob") from _call_mc_sex_request_mom_weekly_pay
...
label mom_weekly_pay_tit_fuck_label():
    $ the_person = mom
    ...
    if (not the_person.has_taboo("touching_body")) or the_person.effective_sluttiness("touching_body") >= 40:
        ...
        "You pull out your wallet and count out her cash in front of her on the table."
        $ mc.business.change_funds(-200)
        if not the_person.tits_visible:
            the_person "Let's unpack these beauties."
            $ the_person.strip_to_tits(position = "sitting")
            $ mc.change_arousal(10)
            the_person "Do you like how they look?"
        "[the_person.possessive_title] gets onto her knees in front of you."
        $ the_person.draw_person(position = "blowjob")
        ...
        if not the_person.tits_visible:
            mc.name "Sure you do [the_person.title]. Just take your tits out and I'll do the rest."
    ...
    $the_person.add_situational_obedience("deal", 20, "I'm doing this for the family")
    call fuck_person(the_person, private = True, start_position =tit_fuck, start_object = make_floor(), skip_intro = True, position_locked = True) from _call_fuck_jennifer_modded_0102
    $ the_person.clear_situational_obedience("deal")
    $ the_report = _return
    if the_report.get("girl orgasms", 0) > 0:
        "You pull up your pants while [the_person.possessive_title] is on her knees panting, trying to get her breath back."
    ...
    if the_mom.love > 70 and the_mom.effective_sluttiness() > 60: #High slut, she offers to fuck you (with slut bonus) if you stay at home
        ...
        menu:
            "Let her \"help\" you":
                ...
                if not the_mom.tits_visible and (the_mom.effective_sluttiness(["underwear_nudity","bare_tits"]) > 25 or the_mom.get_opinion_score("showing her tits") > 0):
...
init -2 python:
    ...
    def get_mom_secretary():
        ...
        return Person.get_person_by_identifier(identifier)
