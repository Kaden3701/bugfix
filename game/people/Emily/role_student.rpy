...
init 1 python:
    ...
    def emily_oral_position_filter(oral_positions):
        # at least one blowjob punishment
        return emily.event_triggers_dict.get("study_blowjob_level", 0) != 0

    def emily_vaginal_position_filter(vaginal_positions):
    ...
    def emily_anal_position_filter(anal_positions):
...
init -2 python:
    ...
    def student_test_requirement(the_person):
        if not the_person.event_triggers_dict.get("student_exam_rewrite_enabled", False):
            return False
        elif not the_person.location == university:
...
label student_wants_serum(the_person):
    if the_person.event_triggers_dict.get("student_wants_serum", False): #TODO: Hook up this trigger. Basically if she's had serum before and it made her smarter.
        ...
        menu:
            "Give her a dose of serum" if mc.inventory.total_serum_count > 0:
        ...
        menu:
            ...
            "Give her a dose of serum" if the_person.obedience >= 110 and mc.inventory.total_serum_count > 0:
            ...
            "Give her a dose of serum\n{color=#ff0000}{size=18}Requires: 110 Obedience{/size}{/color} (disabled)" if the_person.obedience < 110 or mc.inventory.total_serum_count == 0:
                pass
    return
...
label student_study_home(the_person):
    ...
    if the_person.location == christina.home:
...
label student_pick_punishment(the_person):
    ...
    menu:
        ...
        "Strip" if the_person.effective_sluttiness() >= 30:
            if the_person.effective_sluttiness() > 65 or the_person.get_opinion_score("not wearing anything") > 1:
                $ wants_to_fail = True
        ...
        "Spank her" if the_person.effective_sluttiness() >= 30 and the_person.obedience >= 120:
            if the_person.effective_sluttiness() >= 65 or the_person.get_opinion_score("being submissive") > 1:
                $ wants_to_fail = True
        ...
        "Oral punishment" if the_person.effective_sluttiness() >= 40 and the_person.obedience >= 130:
            if the_person.effective_sluttiness() >= 80 or the_person.get_opinion_score("giving blowjobs") > 1:
                $ wants_to_fail = True
...
label student_test(the_person): #TODO: Hook this up
    ...
    if passed:
        ...
        menu:
            "Offer her a job" if mc.business.employee_count < mc.business.max_employee_count:
