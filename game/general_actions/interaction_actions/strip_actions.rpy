init 1 python:
    def get_strip_menu_options(person):
        ...
        for item in person.outfit.get_unanchored():
            ...
            new_willingness = person.effective_sluttiness("underwear_nudity") + (5*person.get_opinion_score("not wearing anything")) - test_outfit.outfit_slut_score
...
label pay_strip_scene(the_person):
    ...
    $ strip_willingness = the_person.effective_sluttiness("underwear_nudity") + (5*the_person.get_opinion_score("not wearing anything")) - the_person.outfit.outfit_slut_score
    ...
    while keep_stripping:
        ...
        if strip_choice == "Watch":
            if renpy.random.randint(0,1) == 0:
                ...
                if renpy.random.randint(0,100) < free_spirit_threshold: #She's independent enough to strip, change pose, etc. on her own.
                    if tease_item is not None and new_willingness >= (the_person.obedience-100): #A more obedient person is less willing to strip without being told to. A less obedient person will strip further on their own.
                        ...
                        $ new_willingness = the_person.sluttiness + (5*the_person.get_opinion_score("not wearing anything")) - test_outfit.outfit_slut_score
                        $ price = 0
                        $ taboo_break = False
            ...
            $ strip_willingness = the_person.effective_sluttiness("underwear_nudity") + (5*the_person.get_opinion_score("not wearing anything")) - test_outfit.outfit_slut_score
    ...
    def get_strip_tease_menu_options(person, guy_state, girl_state):
        ...
        for next_position in girl_state.leads_to:
            if person.effective_sluttiness() < next_position[0].slut_requirement:
                command_options.append([next_position[1] + "\n{color=#ff0000}{size=18}Requires: " + get_gold_heart(next_position[0].slut_requirement) + "{/size}{/color} (disabled)", next_position[0]])
            else:
    ...
    while should_continue and not the_person is None:
        ...
        if the_choice == "description": #NOTE: Not an else-if, this is always run if the results above call for a description.
            ...
            if girl_state.girl_energy_cost > the_person.energy:
                ...
                $ clarity_added = (girl_state.girl_arousal_gain/2) + the_person.sex_skills["Foreplay"] #Note that this _isn't_ exponential like actual sex is.
        ...
        if the_person.effective_sluttiness() + 5 * the_person.get_opinion_score("being covered in cum") < 25: #Not willing no matter what.
            ...
            menu:
                "Pay her\n{color=#ff0000}{size=18}Costs: $[cum_cost]{/size}{/color}" if mc.business.has_funds(cum_cost) and for_pay:
                    ...
                    $ the_person.change_obedience(-(1 + the_person.get_opinion_score("being submissive"))) #She loses obedience, more if she likes being submissive (you're begging her!)
