...
init -2 python:
    ...
    def seduce_requirement(the_person):
        if the_person.sluttiness < 15:
            return "Requires: {{{}}}".format(get_gold_heart(15))
        return True
    ...
    def bc_talk_requirement(the_person):
        if persistent.pregnancy_pref == 0 or the_person.fertility_percent <= -100:
            return False
        elif the_person.effective_sluttiness() < 20 and the_person.love < 20:
            return False
        elif the_person.is_pregnant: # don't talk about bc when she is pregnant
            return False
        return True
    ...
    def make_onlyfans_together_requirement(person):
        if not the_person.has_role(onlyfans_role):
            return False
        if not person.is_employee and not the_person.is_home:
            return "Only at her place"
        if person.is_employee and not (the_person.is_at_work or the_person.is_home):
    ...
    def bend_over_your_desk_requirement(person):
        if not (the_person.is_employee and the_person.is_at_work):
            return False
    ...
    def demand_bc_requirement(the_person):
        if persistent.pregnancy_pref == 0 or the_person.fertility_percent <= -100: #Don't talk about pregnancy if we don't want any of it.
            return False
...
label small_talk_person(the_person, apply_energy_cost = True, is_phone = False): #Tier 0. Useful for discovering a character's opinions and the first step to building up love.
    python:
        ...
        renpy.say(mc.name, "So [the_person.title], what's been on your mind recently?")
        the_person.discover_opinion("small talk")

    # TODO: Add a chance that she wants to talk about someone she knows.
    if not is_phone and the_person.love > 5 and not the_person.event_triggers_dict.get("job_known", True):
        if the_person.job == unemployed_job:
    ...
    elif renpy.random.randint(0,100) < 60 + (the_person.get_opinion_score("small talk") * 20) + (mc.charisma * 5):
        if is_phone:
        ...
        if not opinion_learned is None:
            ...
            $ the_person.change_stats(happiness = the_person.get_opinion_score("small talk") + 1, love = 3 - prediction_difference, max_love = 35)
        ...
        if the_person.get_opinion_score("small talk") >= 0:
            ...
            if is_phone:
                ...
                "[the_person.possessive_title] trails off awkwardly."
    else:
        if the_person.get_opinion_score("small talk") < 0:
            ...
            if is_phone:
                ...
                "[the_person.possessive_title] seems happy to chitchat, and you spend half an hour just hanging out."
...
label compliment_person(the_person): #Tier 1. Raises the character's love. #TODO: just have it raise love and not sluttiness.
    ...
    $ the_person.change_stats(happiness = 2, love = 5, max_love = 20)
