...
init -2 python:
    ...
    def lunch_date_create_topics_menu(the_person):
        ...
        neutral_opinion = get_random_from_list([x for x in the_person.get_normal_opinions_list() if x not in opinion_question_list and the_person.get_opinion_score(x) == 0])
        if neutral_opinion:
...
label date_conversation(the_person):
    ...
    if score > 0:
        ...
        $ kiss_after = True
        $ the_person.change_stats(happiness = 5, love = 8, max_love = 50)
    elif score == 0:
        ...
        $ the_person.change_love(4, max_modified_to = 40)
    else: #Negative score
...
label lunch_date_label(the_person): #Could technically be included in the planning phase, but broken out to fit the structure of the other events.
    ...
    if renpy.random.randint(0,100) < 40:
        ...
        menu:
            "Add some serum to her food" if mc.inventory.total_serum_count > 0:
...
label movie_date_label(the_person):
    ...
    $ the_person.planned_outfit = the_person.decide_on_outfit(.2)
    $ the_person.apply_outfit(the_person.planned_outfit)
...
label dinner_date_label(the_person):
    ...
    menu:
        ...
        "An expensive restaurant\n{color=#ff0000}{size=18}Costs: $300{/size}{/color}" if mc.business.has_funds(300):
            ...
            $ the_person.change_stats(happiness = 5, love = 10)
    ...
    $ the_person.change_stats(happiness = mc.charisma, love = mc.charisma)
    $ the_person.draw_person()
    if the_person.has_role(sister_role) or the_person.has_role(mother_role):
    ...
    if the_person.has_role(affair_role):
        ...
        menu:
            ...
            "Turn her down":
                ...
                $ the_person.change_stats(happiness = -20, love = -(2 + the_person.get_opinion_score("taking control")))
...
label shopping_date_food(the_person):
    ...
    menu:
        "Pay for her [name_string]\n{color=#ff0000}{size=18}Costs: $40{/size}{/color}" if mc.business.has_funds(40):
            ...
            menu:
                "Add serum to her drink" if mc.inventory.total_serum_count > 0:
...
label shopping_date_underwear(the_person):
    ...
    $ new_underwear = None
    $ the_person.change_stats(happiness = 5 + the_person.get_opinion_score("lingerie"), love = 1)
...
label shopping_date_changing_room(the_person, new_outfit, changing_type):
    ...
    if waiting_outside:
        ...
        if changing_type == "overwear":
            ...
            $ mc.change_locked_clarity(5)
            $ the_person.draw_person(position = "back_peek")
            if new_outfit.overwear_slut_score > the_person.sluttiness + 5*(the_person.get_opinion_score("skimpy outfits")):
            ...
            if the_person.effective_sluttiness("underwear_nudity") < 20 or the_person.has_taboo("underwear_nudity"):
                if new_outfit.underwear_slut_score < the_person.sluttiness + 5*the_person.get_opinion_score("lingerie"):
...
label shopping_date_inside_changing_room(the_person, new_outfit, changing_type, skip_get_changed = False): #NOTE: skip_get_changed used when an event has already set her outfit properly.
    ...
    $ sluttiness_token = get_gold_heart(30)
